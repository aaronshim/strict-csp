...
export class StrictCsp {
  ...
  */
  ...
  refactorSourcedScriptsForHashBasedCsp(enableTrustedTypes = false): void {
    ...
    const loaderScript = StrictCsp.createLoaderScript(srcList, enableTrustedTypes);
    if (!loaderScript) {
      return;
    }
   ...
   */
  static createLoaderScript(srcList: string[], enableTrustedTypes = false): string | undefined {
    if (!srcList.length) {
      return undefined;
    }
    ...
    scripts.forEach(function(scriptUrl) {
      ...
      s.src = policy.createScriptURL(scriptUrl);
